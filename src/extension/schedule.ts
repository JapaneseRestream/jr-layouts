...
export const schedule = async (nodecg: NodeCG) => {
	...
	const applyLatestSchedule = async () => {
		...
		const updated = !_.isEqual(latestSchedule, scheduleRep.value);
		if (updated) {
	...
	const applyCurrentRun = (index: number) => {
		...
		currentRunRep.value = _.cloneDeep(runToApply);
	};
